...
- **`PrettyChat.toc`** loads `GlobalStrings_001.lua` … `GlobalStrings_010.lua` *eagerly at addon startup* (load order: after `defaults/`, before `modules/Override.lua` and the `settings/` files). This populates `ns.GlobalStrings` so `settings/Panel.lua`'s "Original Format String" disabled input can resolve every key without an explicit load step.

...
- **Cyan `[PC]` chat prefix on all addon output.** Routes through `ns.Print(msg)` in `core/PrettyChat.lua`, which prepends `ns.PREFIX` (`|cff00ffff[PC]|r `). **No raw `print(...)` calls.** `Test()` (in `modules/Override.lua`) writes directly to `DEFAULT_CHAT_FRAME:AddMessage` but every body line still starts with `PREFIX`. Gated developer logging goes through `ns.Debug(tag, fmt, ...)`, which routes to the on-screen debug console (`core/DebugLog.lua`, `ns.DebugLog`) — zero-alloc no-op when off, gated on session-only `ns.State.debug` (`core/State.lua`), toggled through the single `ns.DebugLog:SetEnabled(on)` seam and driven by `/pc debug`. Current `ns.Debug` producers: `PrettyChat:ApplyStrings` (`modules/Override.lua`, tag `Apply` — applied/restored counts) and `Schema.Set` (`settings/Schema.lua`, tag `Set` — every schema write). Add more `ns.Debug("<Tag>", …)` calls at any point worth tracing.
